...
local LD_LIBRARY_PATH   = os.getenv("LD_LIBRARY_PATH")  or "$LD_LIBRARY_PATH"
local PYTHON_LIBPATH    = os.getenv("PYTHON_LIBPATH")  or "$PYTHON_LIBPATH"
...
DEFAULT_SCRIPT_FILENAME	 = "/tmp/dummy.py"
default_proxy_options = {
	["pyproxy-backend-addresses"] = MYSQL_HOST .. ":" .. MYSQL_PORT,
	["pyproxy-address"]		   	= PROXY_HOST .. ":" .. PROXY_PORT,
	["admin-address"]		   	= PROXY_HOST .. ":" .. ADMIN_PORT,
	["pid-file"]				= PROXY_PIDFILE,
	["pyproxy-python-script"]		= DEFAULT_SCRIPT_FILENAME,
...
default_master_options = {
	["pyproxy-backend-addresses"] = MYSQL_HOST .. ":" .. MYSQL_PORT,
	["pyproxy-address"]		   	= PROXY_HOST .. ":" .. PROXY_MASTER_PORT,
...
default_slave_options = {
	["pyproxy-backend-addresses"] = MYSQL_HOST .. ":" .. MYSQL_PORT,
	["pyproxy-address"]		   	= PROXY_HOST .. ":" .. PROXY_SLAVE_PORT,
...
function wait_proc_up(pid_file) 
	...
	while not file_exists(pid_file) do
		...
		if rounds > 1000 then error(("pyproxy failed to start: no pid-file %s"):format(pid_file)) end
	end
	...
	while 0 ~= os.execute("kill -0 ".. pid .."  2> /dev/null") do
		...
		if rounds > 5 then error(("pyproxy seems to have crashed: pid=%d (%s)"):format(pid, pid_file)) end
	end
end
...
function before_test(basedir, test_name)
	local script_filename = basedir .. "/t/" .. test_name .. ".py"
	...
	if file_exists( script_filename) then
		if has_option_file then 
			default_proxy_options['pyproxy-python-script'] = script_filename
			print_verbose('using python script directly ' .. script_filename)
			file_empty(DEFAULT_SCRIPT_FILENAME)
		else
			default_proxy_options['pyproxy-python-script'] = DEFAULT_SCRIPT_FILENAME
			--default_proxy_options['pyproxy-python-script'] = script_filename
			file_copy(DEFAULT_SCRIPT_FILENAME, script_filename)
			-- Here is a trick!
			os.remove(DEFAULT_SCRIPT_FILENAME .. 'c')
			print_verbose('copying python script to default ' .. script_filename)
		end
	else
		default_proxy_options['pyproxy-python-script'] = DEFAULT_SCRIPT_FILENAME
		file_empty(DEFAULT_SCRIPT_FILENAME)
		print_verbose('using empty python script')
	end
	global_basedir = basedir
	print_verbose ('current_dir ' ..  
					basedir ..  
					' - script: '  ..
					default_proxy_options['pyproxy-python-script']  )
	-- 
...
function run_test(filename, basedir)
	...
	local ret = conditional_execute(test_string)
...
function sql_execute(queries, proxy_name)
	...
	if proxy_name then  
		...
		assert(opts['pyproxy-address'],'address for proxy '.. proxy_name .. ' not found')
		local p_host, p_port = opts['pyproxy-address']:match('(%S+):(%S+)')
...
function start_proxy(proxy_name, proxy_options)
	...
	if not file_exists(proxy_options['pyproxy-python-script']) then
		proxy_options['pyproxy-python-script'] = 
			global_basedir .. 
			'/t/' ..  proxy_options['pyproxy-python-script'] 
	end
	...
	if proxy_options['pyproxy-address'] == '/tmp/mysql-proxy-test.sock' then
		os.remove(proxy_options['pyproxy-address'])
	end
	-- os.execute("head " .. proxy_options['proxy-lua-script'])
    
	print_verbose( 'LUA_PATH="' .. INCLUDE_PATH  .. '"  LD_LIBRARY_PATH="'  .. LD_LIBRARY_PATH .. '" PYTHON_LIBPATH="' .. PYTHON_LIBPATH .. '" ' ..
		PROXY_TRACE .. " " .. PROXY_BINPATH .. " " ..
		options_tostring( proxy_options) .. " --plugins=pyproxy --plugins=admin " .. " &"
	)
	--assert(os.execute( 'LUA_PATH="' .. INCLUDE_PATH  .. '"  LD_LIBRARY_PATH="' .. LD_LIBRARY_PATH .. '"  ' ..
	--	PROXY_TRACE .. " " .. PROXY_BINPATH .. " " ..
	--	options_tostring( proxy_options) .. " --plugins=pyproxy --plugins=admin " .. " &"
	--))
	assert(os.execute( 'LUA_PATH="' .. INCLUDE_PATH  .. '"  LD_LIBRARY_PATH="' .. LD_LIBRARY_PATH .. '" PYTHON_LIBPATH="' .. PYTHON_LIBPATH .. '" ' ..
		PROXY_TRACE .. " " .. PROXY_BINPATH .. " " ..
		options_tostring( proxy_options) .. " --plugins=pyproxy --plugins=admin " .. " &"
	))
...
-- @param second_python_script 
-- @param use_replication uses a master proxy as backend 
function chain_proxy (backend_lua_scripts, second_python_script, use_replication)
	local backends = { }
	...
	for i, backend_lua_script in ipairs(backends) do
		...
		backend_proxy_options = {
			["pyproxy-backend-addresses"] = MYSQL_HOST .. ":" .. MYSQL_PORT,
			["pyproxy-address"]		   = backend_addresses[i],
			["admin-address"]		   = PROXY_HOST .. ":" .. (ADMIN_CHAIN_PORT + i - 1),
			["pid-file"]				= PROXY_CHAIN_PIDFILE .. i,
			["pyproxy-python-script"]		= backend_lua_script or DEFAULT_SCRIPT_FILENAME,
			["plugin-dir"]			= PROXY_LIBPATH,
			["basedir"]					= PROXY_TEST_BASEDIR,
		}
		-- 
		...
		if use_replication and (use_replication == true) then
			...
			backend_proxy_options["pyproxy-backend-addresses"] = PROXY_HOST .. ':' .. PROXY_MASTER_PORT
		end
		start_proxy('backend_proxy' .. i, backend_proxy_options) 
	end

	second_proxy_options = {
			["pyproxy-backend-addresses"] = backend_addresses ,
			["pyproxy-address"]		   	= PROXY_HOST .. ":" .. PROXY_PORT,
			["admin-address"]		   	= PROXY_HOST .. ":" .. ADMIN_PORT,
			["pid-file"]				= PROXY_PIDFILE,
			["pyproxy-python-script"]		= second_python_script or DEFAULT_SCRIPT_FILENAME,
			["plugin-dir"]			= PROXY_LIBPATH,
			["basedir"]					= PROXY_TEST_BASEDIR,
	}
